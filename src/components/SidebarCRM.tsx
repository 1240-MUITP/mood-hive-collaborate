...
export default function SidebarCRM({
  ...
  return <nav className="h-screen sticky top-0 flex flex-col bg-[#13151A] text-white min-w-[255px] max-w-[255px] pb-6 border-r border-[#23242A] select-none">
      ...
      <ul className="flex-1 flex flex-col gap-1 px-2 text-base">
        ...
        return <li key={s.label}>
              <button className={cn("flex items-center gap-3 px-4 py-2 rounded text-left font-medium tracking-tight transition-all w-full", active ? "bg-[#192039] text-white shadow-md border border-blue-400" : "text-gray-300 hover:bg-[#191c22]")} style={active ? {
            boxShadow: "0 0 12px 1px #2051eb30"
          } : undefined} aria-current={active ? "page" : undefined} onClick={() => setSection(s.label)}>
                {s.icon === "dot" && <span className={"inline-block w-2 h-2 rounded-full " + s.color} />}
                {s.icon === "cube" && <span>
                ...
                <span>{s.label}</span>
                {active && <span className="w-3 h-3 rounded-full bg-blue-500 shadow-[0_0_8px_2px_rgba(37,99,235,0.25)] ml-auto" />}
              </button>
            </li>;
      })}
      </ul>
